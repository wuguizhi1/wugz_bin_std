...
percent <- function(data=NA, max=0, min=0){
	...
	for (i in 1:ncol(re)){
		...
		ratiofile = paste0(indir,"/statistic/05.variant_result/",each,".fusion.final.txt")
		if( file.exists(ratiofile)  ){
			...
			EM =data.frame(Sample = each, MutID = "SEA", MutTotal = round(seaT*adjust2, 0), DepTotal = seaT, FreqTotal = adjust2)
