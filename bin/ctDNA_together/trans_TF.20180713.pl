...
while(<IN>){
	chomp;
	my @line = split/\t/, $_; 
	my( $SampleID,$SampleName, $Sample,$Fetal_E,$Mother_E,$Father_E, $Geno,$Total,$Mutation,$Ratio,$cffDNA,$Genotype,$Pvalue ) = @line[0..1,7..10,14..20];
	print join "\t", ( $SampleID,$SampleName, $Sample,$Fetal_E,$Mother_E,$Father_E, $Geno,$Total,$Mutation,$Ratio,$cffDNA,$Genotype,$Pvalue );
	print "\n";
	...
	$people_geno{$sample}{"1"} = $Mother_E;
	$people_geno{$sample}{"2"} = $Fetal_E;
	$people_geno{$sample}{"3"} = $Father_E;
...
foreach my$S(keys %Sample_Fetal_Geno){ 					#print "$S\n";
	...
	foreach my$G( keys %$Fetal_Geno ){					#print "\t$G\n";
		if( exists $Ratio_high{$S}{$G} and $G ne "N" ){	
			my @print_line = @{$Ratio_high{$S}{$G}};	print "$S\t$G\thigh\t@print_line\n";
			...
			my @print_line = @{$Ratio_low{$S}{$G}};		print "$S\t$G\tlow@print_line\n";
			my( $re, $Mother_E, $Fetal_E, $Father_E ) = &Geno_JianYan(@print_line[0..2,9,10]);
			my $p = join "\t", (@print_line, $Mother_E, $Fetal_E, $Father_E);
			print OUT "$p\tFN\n";
		}
