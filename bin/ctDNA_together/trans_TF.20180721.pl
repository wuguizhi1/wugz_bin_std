...
while(<IN>){
	chomp;
	my @line = split/\t/, $_; 
	my( $SampleName, $Sample, $Geno, $Total, $Mutation, $Ratio, $cffDNA ,$Genotype, $Pvalue ) = @line[1,6..13];
	...
	my @print_line = ( $SampleName, $Sample, $Geno, $Total, $Mutation, $Ratio, $cffDNA , $Genotype, $Pvalue);
	@print_line = ( @print_line,$Mother, $Fetal, $Mother_E, $Fetal_E, $Father_E );
	if( $SampleName ne "SampleName" and $Ratio > 0.01 ){
		...
		if( not exists $Ratio_high{$SampleName}{$Geno} ){
			...
			if( $print_line[6] > $print_line_old[6] ){ print "$print_line[6] > $print_line_old[6]\t$SampleName:$Geno";
				@{$Ratio_high{$SampleName}{$Geno}} = ($re,@print_line);
			}
		}
	}else{ 
